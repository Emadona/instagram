...
class Osintgram:
    ...
    def get_fwersinfo(self):
        ...
        try:
            ...
            for follow in followers:
                ...
                if 'following_count' and user['user']['following_count']:
                    follow['following_count'] = user['user']['following_count']
                
                if 'follower_count' and user['user']['follower_count']:
                    follow['follower_count'] = user['user']['follower_count']
                if 'hd_profile_pic_versions' and user['user']['hd_profile_pic_versions']:
                    follow['hd_profile_pic_versions'] = user['user']['hd_profile_pic_versions'][1]['url']
                if 'address_street' and user['user']['address_street']:
                    follow['address_street'] = user['user']['address_street']
                if 'address_street' and user['user']['address_street']:
                    follow['address_street'] = user['user']['address_street']
                if 'city_name' and user['user']['city_name']:
                    follow['city_name'] = user['user']['city_name']
                if 'public_email' and user['user']['public_email']:
                    follow['public_email'] = user['user']['public_email']
                if 'public_phone_number' and user['user']['public_phone_number']:
                    follow['public_phone_number'] = user['user']['public_phone_number']
                
                if 'contact_phone_number' and user['user']['contact_phone_number']:
        ...
        if len(results) > 0:
            ...
            if self.jsonDump:
                ...
                with open(json_file_name, 'w') as f:
                    json.dump(json_data, f)
            csv_file_name = self.output_dir + "/" + self.target + "_fwersbio.csv"
            dt = pd.DataFrame(results)
            dt.to_csv(csv_file_name)
            print(t)
    ...
    def get_fwersfollowers(self):
        ...
        try:
            ...
            pc.printout("Searching for followers of target followers... this can take a few minutes\n")
            ...
            next_max_id = data.get('next_max_id')
            while next_max_id:
                sys.stdout.write("\rCatched %i followers followers" % len(followers))
                sys.stdout.flush()
                results = self.api.user_followers(str(self.target_id), rank_token=rank_token, max_id=next_max_id)
            ...
            pc.printout("Do you want to get all followers? y/n: ", pc.YELLOW)
            value = input()
            ...
            if value == str("y") or value == str("yes") or value == str("Yes") or value == str("YES"):
                ...
                while True:
                    try:
                        pc.printout("How many followers do you want to get? ", pc.YELLOW)
                        new_value = int(input())
                        value = new_value - 1
                        break
                    except ValueError:
        ...
        if len(results) > 0:
            ...
            t = PrettyTable(['ID', 'Username', 'Full Name', 'Followers'])
            t.align["ID"] = "l"
            t.align["Username"] = "l"
            t.align["Full Name"] = "l"
            t.align["Followers"] = "l"
            ...
            for node in results:
                t.add_row([str(node['id']), node['username'], node['full_name'], node['follower_count']])

            if self.writeFile:
                file_name = self.output_dir + "/" + self.target + "_fwersfollowers.txt"
                file = open(file_name, "w")
                file.write(str(t))
                file.close()

            if self.jsonDump:
                json_data['followers_followers'] = results
                json_file_name = self.output_dir + "/" + self.target + "_fwersfollowers.json"
                with open(json_file_name, 'w') as f:
                    json.dump(json_data, f)
